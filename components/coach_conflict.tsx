...
export default function CoachPanel() {
  ...
  return (
    <section style={panelStyle} className="fade-up">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', marginBottom: collapsed ? 0 : 14 }}>
        <div>
          ...
          {/* Preview sintesi quando collapsed e risposta già caricata */}
          {response && collapsed ? (
            <div className="mono" style={{ fontSize: 10, color: 'var(--text-dim)', marginTop: 5 }}>
              {response.summary.length > 90 ? response.summary.slice(0, 90) + '…' : response.summary}
            </div>
          ) : (
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          {response && (
            <>
              <StatusPill risk={response.risk_level} />
              <span
                className="mono"
                style={{
                  fontSize: 9,
      ...
      {/* ── TUTTO IL RESTO: visibile solo se !collapsed ── */}
      {!collapsed && (
        <>
          {/* Bottoni azione */}
